    ...
    var drawFunction = function () {
        ...
        for (var i = 0; i < relateD.length; i++) {
            ...
            pathelement.attr('d', path.toString())
                .append("title").text(data.A.name + "\nRange : " + data_A[i][1] + "-" + data_A[i][2] +
                "\nLength : " +     Math.abs(data_A[i][1]- data_A[i][2])
                + "\n||||\n" +
                data.B.name + "\nRange : " + b1 + "-" + b2 +
                "\nLength : " + Math.abs(b1-b2)
            );
        }
...
function drawBar2(seq,bar,data,gen,index,max3) {
    ...
    const ul = "<li role='presentation' class='" + cs + "' ><a href='#bar" + bar + "-" + num + "' id='" + bar + "-" + num + "-tab' role='tab' data-toggle='tab' " +
        "aria-controls='" + bar + "-" + num + "' aria-expanded='true'>" + bar + "</a></li>";
    $("#bar" + seq + " #myTab").append(ul);

    const li = "        <div role=\"tabpanel\" class=\"tab-pane fade in " + cs + "\" id='bar" + bar + "-" + num + "' aria-labelledby='bar" + bar + "-" + num + "-tab'>\n" +
        "        </div>";

    $("#bar" + seq + " #myTabContent").append(li);
    const barid = "#bar" + seq + " #bar" + bar + "-" + num;
    var width = 1000;
    var height = 500;
    var rectWidth = 4;
    //画布周边的空白
    var padding = 40;
    var svg = d3.select(barid).append('svg').attr('width',width).attr('height',height);
    ...
    svg.selectAll("rect")
        ...
        .attr("y",padding+5)
        .attr("width","1")
        .attr("height",d=>yScale(d.value)-10)
        .attr("class","myrect");
    ...
    svg.selectAll("bar")
        ...
        .enter()
        .append("a")
        .attr("href", d=>"#hit"+d.hitid)
        .append("rect")
        .attr("x",d=>xScale(d.key) + 6)
        .attr("y",d=>yScale(d.value) + padding -2)
        .attr("width","20")
        .attr("height","4")
        .attr("class","bar")
